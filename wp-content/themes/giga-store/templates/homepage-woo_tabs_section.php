					...
					<div class="tab-content">
						...
						<?php foreach ( $sortable_value as $checked_value ) : ?>	
							<div class="tab-pane <?php if ( $i == 0 ) { echo 'active'; } ?>" id="<?php echo esc_attr( $checked_value ); ?>" data-container="<?php echo esc_attr( $checked_value ); ?>" >
								<?php echo do_shortcode( '[' . $checked_value . ' per_page="' . absint( get_theme_mod( 'tabs_products_per_row', '3' ) ) . '" columns="' . absint( get_theme_mod( 'tabs_products_per_row', '3' ) ) . '" orderby="date" order="ASC"]' ) ?>
							</div>
							<?php $i++; ?>
						<?php endforeach; ?>
					</div>
